...
def image_to_text(path):
    ...
    reader = easyocr.Reader(['en']) #en is english
...
if  select == "Home":
    ...
    if img is not None:
        ...
        #creating dataframe for image and concat(image df & extra data)

        df_1= pd.DataFrame(data)

        concat_df = pd.concat([df,df_1],axis=1)#axis 0 is row
        ...
        if button_1:
            ...
            #drop_query = "drop table if exists bizcard_details"
            #mycursor.execute(drop_query)

            #Table creation
            create_table_query = '''CREATE TABLE IF NOT EXISTS bizcard_details(name varchar(255),designation varchar(255),company_name varchar(225),contact varchar(225),
                                    email varchar(225),website text,address text,pincode varchar(225),image LONGBLOB)'''
            ...
            insert_query = '''INSERT INTO bizcard_details(name,designation,company_name,contact,email,
    ...
    if method == "None":
        ...
        col1,col2 = st.columns(2)
        with col1:
            mo_name = st.text_input("Name",df_3["NAME"].unique()[0]) #only that column-> unique
        ...
        with col2:
            mo_website = st.text_input("Website",df_3["WEBSITE"].unique()[0]) #only that column-> unique
